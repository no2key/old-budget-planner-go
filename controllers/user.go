...
import (
	"budget-planner-go/models"
	pk "budget-planner-go/utilities/pbkdf2"
	"encoding/hex"
	"fmt"
...
func sendVerification(email, u string) bool {
	...
	link := beego.AppConfig.String("domain") + "user/verify/" + u
	host := "smtp.gmail.com"
	port := 587
	msg := gomail.NewMessage()
	from_email := beego.AppConfig.String("from_email")
	from_name := beego.AppConfig.String("from_name")
	//msg.SetAddressHeader("From", "acmecorp@gmail.com", "ACME Corporation")
	msg.SetAddressHeader("From", from_email, from_name)
	msg.SetHeader("To", email)
	//msg.SetHeader("Subject", "Account Verification from ACME Corporat")
	...
	m := gomail.NewMailer(host, beego.AppConfig.String("gmail_user"), beego.AppConfig.String("gmail_pass"), port)
